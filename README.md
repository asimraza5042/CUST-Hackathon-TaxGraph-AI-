🕸️ TaxGraph AI — Pakistan FBR Intelligence Platform
...
🔒 Synthetic data only. All names, CNICs, addresses, and records in this repo
...
Entity Resolution — fuzzy-matches people across four independent sources
...
Knowledge Graph Construction — builds a directed graph (Person → Vehicle,
Person → Meter, Person → Tax Filing) using NetworkX, capturing how each
individual is cross-linked across databases.
Tax Compliance Deviation Scoring — estimates a "lifestyle income" proxy from
...
Interactive Dashboard — a self-contained HTML app for exploring entities,
...
bashpip install pandas networkx
...
bashpython generate_entities_js.py
